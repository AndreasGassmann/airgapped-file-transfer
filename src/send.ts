...
const setupSlider = (sliderElName: string, sliderValueName: string) => {
    let slider: HTMLInputElement = document.getElementById(sliderElName) as any;
...
function unicodeStringToTypedArray(s: string) {
    ...
    var binstr = escstr.replace(/%([0-9A-F]{2})/g, function (_match: any, p1: any) {
 ...
 */
function selectFile(contentType, multiple): Promise<File[]> {
...
const startEncodingData = () => {
    ...
    const textAreaValue = (document.getElementById('data') as any).value
    const ur = textAreaValue ? bc.UR.fromUint8Array(unicodeStringToTypedArray(textAreaValue)) : bc.UR.fromUint8Array(fileReadData);

    const maxFragmentLength = Number((document.getElementById('size') as any).value);
    const firstSeqNum = 0;
...
const startInterval = () => {
    ...
    interval = setInterval(() => {
        drawQR(encoder.nextPart())
    }, Number((document.getElementById('speed') as any).value))
}
